...
echo "==================================================="
echo "  HestiaCP Interactive WordPress Cloner"
echo "==================================================="
...
select OLD_WEB_DOMAIN in "${DOMAINS[@]}"; do
  if [ -n "$OLD_WEB_DOMAIN" ]; then
    OLD_DOMAIN="$OLD_WEB_DOMAIN" # Keeping old var reference for URLs
    break
  else
...
# Define Paths & Variables based on interactive selections
...
# Function to check and enable bash shell
ensure_bash() {
...
# Function to check disabled PHP CLI functions
check_php_cli() {
  local USER_TO_CHECK=$1
...
# Crucial for cross-user cloning
