...
- **`v-wp-redis-install.sh`** — installs the Redis Cache plugin for a domain, sets a unique `WP_REDIS_PREFIX` in wp-config, and activates the plugin. Does not enable the object cache drop-in (that is a separate step). Flags: `--user` `--domain`. Also sets two constants that only matter at scale, never overwriting existing values: **`WP_REDIS_DISABLE_GROUP_FLUSH=true`** — the redis-cache plugin's `flush_group()` runs a Lua `SCAN` over the *entire* keyspace on every call, gated by nothing except this constant (not the prefix, not the database, not `WP_REDIS_SELECTIVE_FLUSH`); with it set, group flush falls back to `FLUSHDB`, which is O(1) and invalidates more than asked rather than less, so it is conservative. **`WP_REDIS_MAXTTL=86400`** — without it keys never expire and the keyspace only grows, which is what makes each scan expensive. On kuumalahde.fi (2026-08-30) the pair of defaults cost 406k keys with 4k TTLs, 392,916 flush scans in 46h at ~173ms each — 41% of a core, and nightly 504s, because Redis is single-threaded and every scan blocked all other cache reads. Small sites never notice; it bites at WooCommerce scale with a write-heavy import. `v-server-audit` reports the same condition on boxes installed before this default.
