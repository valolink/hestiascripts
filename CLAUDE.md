...
- **`setup-restic-backup.sh`** — one-time onboarding of a box to a shared Hetzner Storage Box for restic incremental backups (drives Hestia's own `v-backup-user-restic`). Idempotent, guided: generates a **dedicated per-box** ed25519 key (`/root/.ssh/storagebox`), pins the Storage Box host key, authorizes the key via Hetzner's `install-ssh-key` (interactive, prompts for the Storage Box password once), creates an rclone sftp remote, then registers a **per-box** repo path (`/hestia-<host>/`, so boxes don't collide) with `v-add-backup-host-restic` (which flips `BACKUP_INCREMENTAL=yes`). Re-run after installing the key to finish. Prints the must-do follow-ups: verify the DB is in the snapshot, enable Storage Box snapshots (deletion backstop), copy the restic encryption keys off-box (they live on the box being backed up — lose them and the repo is unrecoverable), keep tarball backups until a test restore. Usage: `setup-restic-backup.sh --host uXXXXX.your-storagebox.de --user uXXXXX`. Not a `v-` script for the same reason as the others — root-only setup, off the HTTP path.
