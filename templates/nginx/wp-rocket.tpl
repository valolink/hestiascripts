server {
    ...
    location / {
        ...
        # revalidating. The default that prevents that is declared at http level
        # in /etc/nginx/conf.d/valolink-html-cache-control.conf, deliberately not
        # here: a $variable referenced by this template but never defined would
        # stop nginx from starting on any box missing that file.
